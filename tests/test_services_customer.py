...
test_profile = {
                ...
                "RegisteredAddress": "1 Main Street,\
                    Anytown, Anycounty, Anyland, XX9 9XX",
                "password": "Secret_Pa55w0rd",
                "signup_ts": None,
                "AreasCovered": ["DT1", "DT2"],
...
test_customer_id = "47bfc611-872d-4f4f-b2f2-7d9e505646ec"

test_customer_profile = {'password': 'Secret_Pa55w0rd',
...
def test_process_registration_valid():
    ...
    assert actual["title"] == 'Spick N Span',\
        "Name not as expected"
...
def test_update_database():
    ...
    SET customer.title = q.title,
    customer.status = q.status,
    customer.CompanyReg = q.CompanyReg,
    customer.RegisteredAddress = q.RegisteredAddress,
    customer.password = q.password,
    customer.signup_ts = q.signup_ts,
    customer.joined = q.joined,
...
def test_get_customer_profile():
    ...
    assert actual['profile'] == expected_profile,\
        f"Expected Error got {actual['result']}"
