...
fake_customers_db = {"9g644301-e3f1-4752-90d5-99fbfad99xy4": {
                ...
                "title": 'ServiceMaster',
                "CompanyReg": 7999999,
                "RegisteredAddress": "The Cleaning Centre,\
                Groove Trading Estate,\
                Dorchester DT1 1ST",
                "password": "Secret_Pa55w0rd",
                "signup_ts": None,
                "AreasCovered": ["DT1", "DT2",
     ...
     "9g644301-e3f1-4752-90d5-99fbfad99dc9": {
                ...
                "title": 'Cloverclean',
                "CompanyReg": 7888888,
                "RegisteredAddress": "27 Oakwood,\
                    Broadmayne, Dorchester DT2 8UL",
                "password": "Secret_Pa55w0rd",
                "signup_ts": None,
                "AreasCovered": ["DT1", "DT2", "DT3",
                                 "DT4", "DT5", "DT6",
                                 "DT7", "DT8"],
...
async def read_all_clients():
    resp = customer.get_all_customers()
    print(f"Customer : {resp}")
    return {"Customer_List": resp["profile"]}
...
async def update_customer(customers_id: str):
    if customers_id != "plumbus":
        raise HTTPException(
            status_code=403,
            detail="You can only update the customers: plumbus"
        )
